# https://github.com/gnrochabr/Banco_Dados

USE revisao;
https://github.com/gnrochabr/Banco_Dados/blob/main/2024_2/REVISAO/REVISAO_DQL.sql
...
from jogos
where ano_lancamento > 2020 #filtragem pois é após
order by ano_lancamento #opcional

-- 4. Liste todos os clientes que moram na cidade de 'Vila Velha'.
# tabelas diferentes, juntar JOIN
#INNER é uma intersecção de valores de pessoas e tal

SELECT nome #sempre coloca no from uma tabela que tem elementos no select
from clientes -- AS c
JOIN enderecos ON clientes.endereco_id = enderecos.endereco_id  #ou innerjoin
#to atrelando uma informação a outra, das tabelas com o ON
...
# primeiro saber quem são
	select nome, data_nascimento  from(
	select cliente_id, nome, data_nascimento
	from clientes
	order by cliente_id desc -- maior para o menor
	limit 5) as ultimos_cadastros
order by data_nascimento desc;
)
-- 8. Calcule a quantidade de locações feitas por cada cliente.
-- apareceu count, avg ... tem group by
select c.nome, COUNT(l.cliente_id) as total_locacoes -- quantas vezes o id do cliente apareceu na locacao
from locacoes l
right join clientes c on c.cliente_id = l.cliente_id
-- minha tabela de referencia ta depois 
-- RIGHT JOIN: Clientes vem depois de locações
-- Toda vez que for uma tabela que eu quero seleconar valores nulos também
-- precisamos observar a posição dela no código
-- se tiver antes da tabela que tem NULL é LEFT JOIN, se tiver depois é  

-- 9. Encontre os jogos mais alugados (com pelo menos 5 locações).
select j.titulo, COUNT(l.jogo_id) AS 'n° de '
